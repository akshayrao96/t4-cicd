...
class RepoManager:
    ...
    def _get_yaml_files(self) -> List[str]:
        ...
        if not os.path.isdir(cicd_dir):
            logger.error("Directory '.cicd-pipelines' not found at current directory.")
            raise FileNotFoundError(f"Directory '.cicd-pipelines' not found at {self.target_path}")
        ...
        return yaml_files
    def _get_yaml_path(self, file_name: str = "pipelines.yml") -> Optional[str]:
    ...
    def parse_yaml(self, file_name: str = "pipelines.yml") -> dict:
        ...
        if yaml_path:
            try:
                ...
                logger.error("Failed to parse YAML file at %s. Error: %s", yaml_path, e)
                raise
        else:
